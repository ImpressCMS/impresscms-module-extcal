...
class IcalendarPersistableObjectHandler extends icms_core_ObjectHandler {
	...
	* @return void
	*/
	function IcalendarPersistableObjectHandler(&$db, $tablename, $classname, $keyname, $idenfierName = false) {
					...
					if($ret[$i][$key] != 0) {
						...
						if(!isset($cached[$externalKey['keyName']][$ret[$i][$key]])) {
							if($externalKey['core']) {
								...
								$handler = icms_getModuleHandler($externalKey['className'], 'icalendar');
							}
