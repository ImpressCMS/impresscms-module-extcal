...
include_once ICMS_ROOT_PATH.'/modules/extcal/class/config.php';

class ExtcalEvent extends icms_core_Object {
...
class ExtcalEventHandler extends ExtcalPersistableObjectHandler {
	...
	function getEventForm($siteSide = 'user', $mode = 'new', $data = null) {
		...
		if($mode == 'edit') {
			...
			$startDateValue = xoops_getUserTimestamp($event->getVar('event_start'), $this->_extcalTime->_getUserTimeZone(icms::$user));
			$endDateValue = xoops_getUserTimestamp($event->getVar('event_end'), $this->_extcalTime->_getUserTimeZone(icms::$user));
		...
		// Category select
		$catSelect = new icms_form_elements_Select(_MD_EXTCAL_CATEGORY, 'cat_id', $cat);
		foreach($cats as $cat) {
		...
		// Description
		$form->addElement(new icms_form_elements_Dhtmltextarea(_MD_EXTCAL_DESCRIPTION, 'event_desc', $desc, 10), false);
		// Max registered member for this event
		$nbMemberElement = new icms_form_elements_Text(_MD_EXTCAL_NBMEMBER, 'event_nbmember', 4, 4, $nbMember);
		...
		// File attachement
		$fileElmtTray = new icms_form_elements_Tray(_MD_EXTCAL_FILE_ATTACHEMENT, "<br />");
		...
		if($siteSide == 'user') {
			$buttonElmtTray->addElement(new icms_form_elements_Button("", "form_preview", _MD_EXTCAL_PREVIEW, "submit"), false);
		}
	...
	{
		$sql = "SELECT event_id, event_title, event_submitter, event_submitdate FROM ".icms::$xoopsDB->prefix("extcal_event")." WHERE event_approved = '1'";
		$authorizedAccessCats = $this->_extcalPerm->getAuthorizedCat($user, 'extcal_cat_view');
		$count = count($authorizedAccessCats);
		if($count > 0) {
			$in = '('.$authorizedAccessCats[0];
	...
	function addEventToCalArray(&$event, &$eventsArray, $startPeriod, $endPeriod) {
		...
		global $extcalTimeHandler, $month, $year;
