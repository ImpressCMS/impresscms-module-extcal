...
include_once ICMS_ROOT_PATH.'/modules/icalendar/class/config.php';

class IcalendarEvent extends icms_core_Object {
...
class IcalendarEventHandler extends IcalendarPersistableObjectHandler {

	var $_icalendarPerm;
	var $_icalendarTime;
	var $_icalendarConfig;

	function IcalendarEventHandler(&$db) {
		$this->_icalendarPerm = IcalendarPerm::getHandler();
		$this->_icalendarTime = IcalendarTime::getHandler();
		$icalendarConfig = IcalendarConfig::getHandler();
		$this->_icalendarConfig = $icalendarConfig->getModuleConfig();
		$this->IcalendarPersistableObjectHandler($db, 'icalendar_event', 'IcalendarEvent', 'event_id');
	}
	...
	function getEvent($eventId, $skipPerm = false) {
		...
		$criteriaCompo->add(new icms_db_criteria_Item('event_id', $eventId));
		$criteriaCompo->add(new icms_db_criteria_Item('event_approved', 1));

		if (!$skipPerm) $this->_addCatPermCriteria($criteriaCompo, icms::$user);
	...
	function formatEventDate(&$event, $pattern) {
		if(!$event['event_isrecur']) {
			$event['formated_event_start'] = $this->_icalendarTime->getFormatedDate($pattern, $event['event_start']);
			$event['formated_event_end'] = $this->_icalendarTime->getFormatedDate($pattern, $event['event_end']);
		} else {
			$event['formated_event_start'] = $this->_icalendarTime->getFormatedDate($pattern, $event['event_start']);
			$event['formated_event_end'] = $this->_icalendarTime->getFormatedDate($pattern, $event['event_end']);
			$event['formated_reccur_rule'] = $this->_icalendarTime->getFormatedReccurRule($event['event_recur_rules']);
		}
		$event['formated_event_submitdate'] = $this->_icalendarTime->getFormatedDate($pattern, $event['event_submitdate']);
	}
	...
	function _userTimeToServerTime(&$data) {
		...
		$data['event_start'] = userTimeToServerTime($data['event_start'], $this->_icalendarTime->_getUserTimeZone($user));
		$data['event_end'] = userTimeToServerTime($data['event_end'], $this->_icalendarTime->_getUserTimeZone($user));
	...
	function serverTimeToUserTime(&$data) {
		...
		$data['event_start'] = xoops_getUserTimestamp($data['event_start'], $this->_icalendarTime->_getUserTimeZone($user));
		$data['event_end'] = xoops_getUserTimestamp($data['event_end'], $this->_icalendarTime->_getUserTimeZone($user));
		$data['event_submitdate'] = xoops_getUserTimestamp($data['event_submitdate'], $this->_icalendarTime->_getUserTimeZone($user));
	...
	function getEventDay($day, $month, $year, $cat=0) {
		...
		$dayStart = userTimeToServerTime(mktime(0,0,0,$month,$day,$year), $this->_icalendarTime->_getUserTimeZone($user));
		$dayEnd = userTimeToServerTime(mktime(23,59,59,$month,$day,$year), $this->_icalendarTime->_getUserTimeZone($user));
	...
	function _getEventWeekCriteria($day, $month, $year, $cat) {
		...
		$weekStart = userTimeToServerTime($userStartTime, $this->_icalendarTime->_getUserTimeZone($user));
		$weekEnd = userTimeToServerTime($userEndTime, $this->_icalendarTime->_getUserTimeZone($user));
	...
	function getEventWeek($day, $month, $year, $cat=0) {
		...
		if(!$this->_icalendarConfig['diplay_past_event_list']) {
			$criteriaCompo->add(new icms_db_criteria_Item('event_end', time(), '>'));
		}
		return $this->getObjects($criteriaCompo);
	}
	...
	function getEventCalendarWeek($day, $month, $year, $cat=0) {
		$criteriaCompo = $this->_getEventWeekCriteria($day, $month, $year, $cat);
		if(!$this->_icalendarConfig['diplay_past_event_cal']) {
			$criteriaCompo->add(new icms_db_criteria_Item('event_end', time(), '>'));
		}
	...
	function _getEventMonthCriteria($month, $year, $cat) {
		...
		$monthStart = userTimeToServerTime($userStartTime, $this->_icalendarTime->_getUserTimeZone($user));
		$monthEnd = userTimeToServerTime($userEndTime, $this->_icalendarTime->_getUserTimeZone($user));
	...
	function getEventYear($year, $cat=0) {
		...
		$yearStart = userTimeToServerTime($userStartTime, $this->_icalendarTime->_getUserTimeZone($user));
		$yearEnd = userTimeToServerTime($userEndTime, $this->_icalendarTime->_getUserTimeZone($user));
	...
	function _getCriteriaCompo($start, $end, $cat, &$user) {
		...
		$criteriaNoRecur = new icms_db_criteria_Compo();
		$criteriaNoRecur->add(new icms_db_criteria_Item('event_start', $end, '<='));
		$criteriaNoRecur->add(new icms_db_criteria_Item('event_end', $start, '>='));
		$criteriaNoRecur->add(new icms_db_criteria_Item('event_isrecur', 0));

		$criteriaRecur = new icms_db_criteria_Compo();
		$criteriaRecur->add(new icms_db_criteria_Item('event_recur_start', $end, '<='));
		$criteriaRecur->add(new icms_db_criteria_Item('event_recur_end', $start, '>='));
		$criteriaRecur->add(new icms_db_criteria_Item('event_isrecur', 1));

		$criteriaCompoDate = new icms_db_criteria_Compo();
	...
	function getUpcommingEvent($nbEvent, $cat = 0) {
		...
		$criteriaNoRecur = new icms_db_criteria_Compo();
		$criteriaNoRecur->add(new icms_db_criteria_Item('event_end', $now, '>='));
		$criteriaNoRecur->add(new icms_db_criteria_Item('event_isrecur', 0));

		$criteriaRecur = new icms_db_criteria_Compo();
		$criteriaRecur->add(new icms_db_criteria_Item('event_recur_end', $now, '>='));
		$criteriaRecur->add(new icms_db_criteria_Item('event_isrecur', 1));
	...
	function getThisDayEvent($nbEvent, $cat = 0) {
		...
		$this->_addCatSelectCriteria($criteriaCompo, $cat);
		$this->_addCatPermCriteria($criteriaCompo, icms::$user);
		$criteriaCompo->add(new icms_db_criteria_Item('event_start', $dayStart, '>='));
		$criteriaCompo->add(new icms_db_criteria_Item('event_start', $dayEnd, '<'));
		$criteriaCompo->add(new icms_db_criteria_Item('event_approved', 1));
		$criteriaCompo->setSort('event_start');
		$criteriaCompo->setLimit($nbEvent);
		return $this->getObjects($criteriaCompo);
	...
	function getPendingEvent() {
		$criteriaCompo = new icms_db_criteria_Compo();
		$criteriaCompo->add(new icms_db_criteria_Item('event_approved', 0));
		$criteriaCompo->setSort('event_start');
		return $this->getObjects($criteriaCompo);
	}
	...
	function _addCatSelectCriteria(&$criteria, $cats) {
		if(!is_array($cats) && $cats > 0) {
			$criteria->add(new icms_db_criteria_Item('cat_id', $cats));
		}
		if(is_array($cats)) {
	...
	function getEventForm($siteSide = 'user', $mode = 'new', $data = null) {
		...
		if($siteSide == 'admin') {
			...
			$action = 'post.php';
			$cats = $catHandler->getAllCat(icms::$user);
		}
		...
		if($mode == 'edit') {
			...
			$formTitle = _MD_ICALENDAR_EDIT_EVENT;
			...
			$startDateValue = xoops_getUserTimestamp($event->getVar('event_start'), $this->_icalendarTime->_getUserTimeZone(icms::$user));
			$endDateValue = xoops_getUserTimestamp($event->getVar('event_end'), $this->_icalendarTime->_getUserTimeZone(icms::$user));
			...
			$formTitle = _MD_ICALENDAR_SUBMIT_EVENT;
		...
		$form = new IcalendarThemeForm($formTitle, 'event_form', $action, 'post', true);
		// Add this extra to allow file upload
		$form->setExtra('enctype="multipart/form-data"');
		// Category select
		$catSelect = new icms_form_elements_Select(_MD_ICALENDAR_CATEGORY, 'cat_id', $cat);
		foreach($cats as $cat) {
			$catSelect->addOption($cat->getVar('cat_id'), $cat->getVar('cat_name'));
		}
		$form->addElement($catSelect, true);
		// Title
		$form->addElement(new icms_form_elements_Text(_MD_ICALENDAR_TITLE, 'event_title', 50, 255, $title), true);
		// Start and end
		new IcalendarFormDateTime($form, $startDateValue, $endDateValue);
		// Description
		$form->addElement(new icms_form_elements_Dhtmltextarea(_MD_ICALENDAR_DESCRIPTION, 'event_desc', $desc, 10), false);
		// Max registered member for this event
		$nbMemberElement = new icms_form_elements_Text(_MD_ICALENDAR_NBMEMBER, 'event_nbmember', 4, 4, $nbMember);
		$nbMemberElement->setDescription(_MD_ICALENDAR_NBMEMBER_DESC);
		$form->addElement($nbMemberElement, false);
		// Contact
		$form->addElement(new icms_form_elements_Text(_MD_ICALENDAR_CONTACT, 'event_contact', 50, 255, $contact), false);
		// Url
		$form->addElement(new icms_form_elements_Text(_MD_ICALENDAR_URL, 'event_url', 50, 255, $url), false);
		// Email
		$form->addElement(new icms_form_elements_Text(_MD_ICALENDAR_EMAIL, 'event_email', 50, 255, $email), false);
		// Address
		$form->addElement(new icms_form_elements_Dhtmltextarea(_MD_ICALENDAR_ADDRESS, 'event_address', $address), false);
		// Recurence form
		$form->addElement(new IcalendarFormRecurRules($reccurOptions));
		// File attachement
		$fileElmtTray = new icms_form_elements_Tray(_MD_ICALENDAR_FILE_ATTACHEMENT, "<br />");

		// If they are attached file to this event
		if(count($files) > 0) {
			$eventFiles = new IcalendarFormFileCheckBox('', 'filetokeep');
		...
		$fileElmtTray->addElement(new icms_form_elements_File(_MD_ICALENDAR_FILE_ATTACHEMENT, 'event_file', 3145728));
		$form->addElement($fileElmtTray);

		if(isset($data['event_id'])) {
			$form->addElement(new icms_form_elements_Hidden('event_id', $data['event_id']), false);
		}

		$buttonElmtTray = new icms_form_elements_Tray('', "&nbsp;");
		$buttonElmtTray->addElement(new icms_form_elements_Button("", "form_submit", _SEND, "submit"), false);
		if($siteSide == 'user') {
			$buttonElmtTray->addElement(new icms_form_elements_Button("", "form_preview", _MD_ICALENDAR_PREVIEW, "submit"), false);
		}
	...
	function getRecurEnd($data, $parm) {
		...
		switch($recurFreq) {
			...
			case 'weekly':
				...
				$offset = date('w',$startWeekTS) - icms::$module->config['week_start_day'];
		...
		switch($eventOptions[0]) {
			...
			case 'daily':
				...
				while($nbOccur < $rrule_daily_interval) {
					...
					if
						// Event start falls within search period
	...
	{
		$sql = "SELECT event_id, event_title, event_submitter, event_submitdate FROM ".icms::$xoopsDB->prefix("icalendar_event")." WHERE event_approved = '1'";
		$authorizedAccessCats = $this->_icalendarPerm->getAuthorizedCat($user, 'icalendar_cat_view');
		$count = count($authorizedAccessCats);
		if($count > 0) {
			$in = '('.$authorizedAccessCats[0];
		...
		while($myrow = icms::$xoopsDB->fetchArray($result)){
			$ret[$i]['image'] = "images/icalendar.gif";
	...
	function addEventToCalArray(&$event, &$eventsArray, $startPeriod, $endPeriod) {

		global $icalendarTimeHandler, $month, $year;
