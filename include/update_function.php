...
function xoops_module_update_extcal(&$xoopsModule, $oldVersion = null) {
	...
	if($oldVersion < 204) {
		...
		$sql = "ALTER TABLE `".$db->prefix('extcal_event')."` ADD `event_nbmember` tinyint(4) NOT NULL default '0' AFTER `event_submitdate` ;";
		$db->query($sql);
	...
	if($oldVersion < 221) {
		...
		$dir_old = ICMS_ROOT_PATH.'/uploads/extcal';
		
		// Rename eXtCal upload directory into Calendar if exist
		if(is_dir($dir_old)) {
			rename($dir_old, $dir);
		}
		...
		if(!is_dir($dir)) {
			...
			$indexFile = ICMS_ROOT_PATH."/modules/extcal/include/index.html";
		...
		$sql = "CREATE TABLE `".$db->prefix('extcal_eventnotmember')."` (`eventnotmember_id` int(11) NOT NULL auto_increment,`event_id` int(11) NOT NULL default '0',`uid` int(11) NOT NULL default '0',PRIMARY KEY  (`eventnotmember_id`),UNIQUE KEY `eventnotmember` (`event_id`,`uid`)) COMMENT='eXtCal By Zoullou' ;";
		$db->query($sql);
	}
