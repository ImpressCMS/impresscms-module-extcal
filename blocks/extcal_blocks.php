...
function _makeXMLSlideshowConf($options) {
	...
	$xml_string = $doc->save(ICMS_ROOT_PATH.'/cache/extcalSlideShowParam.xml');
...
function bExtcalMinicalShow($options) {
	...
	if(!defined('CALENDAR_ROOT')) {
		define('CALENDAR_ROOT', ICMS_ROOT_PATH.'/modules/extcal/class/pear/Calendar/');
	}
	...
	$extcalModuleConfig = $extcalConfig->getModuleConfig();
	...
	*/
	...
	$monthCalObj = new Calendar_Month_Weeks($year, $month, $extcalModuleConfig['week_start_day']);
	$monthCalObj->build();
	...
	for($i=0;$i<$extcalModuleConfig['week_start_day'];$i++) {
	...
	$navig = array(
					...
					'name'=>$extcalTimeHandler->getFormatedDate($extcalModuleConfig['nav_date_month'], $monthCalObj->getTimestamp())
			);
	$ret = array(
...
function bExtcalMinicalEdit($options) {
	...
	$form .= "</select><br /><br />\n";
	$form .= _MB_EXTCAL_SS_WIDTH." <input name='options[2]' type='text' value='".$options[2]."' size='3' maxlength='3'> "._MB_EXTCAL_PX."<br />";
	$form .= _MB_EXTCAL_SS_HEIGHT." <input name='options[3]' type='text' value='".$options[3]."' size='3' maxlength='3'> "._MB_EXTCAL_PX."<br />";
	$form .= _MB_EXTCAL_SS_TRANS_TIME." <input name='options[4]' type='text' value='".$options[4]."' size='2' maxlength='2'> "._MB_EXTCAL_SECONDES."<br />";
	$form .= _MB_EXTCAL_SS_PAUSE_TIME." <input name='options[5]' type='text' value='".$options[5]."' size='2' maxlength='2'> "._MB_EXTCAL_SECONDES."<br />";
	$form .= _MB_EXTCAL_SS_NB_PHOTOS." <input name='options[6]' type='text' value='".$options[6]."' size='2' maxlength='2'><br /><br />";
...
function bExtcalUpcomingShow($options) {
	...
	$eventHandler->formatEventsDate($events, $extcalModuleConfig['event_date_month']);
