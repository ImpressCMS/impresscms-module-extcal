...
function bIcalendarMinicalAddEventToArray(&$event, &$eventsArray, $icalendarTimeHandler, $startMonth, $endMonth) {

	// Calculating the start and the end of the event
	$startEvent = xoops_getUserTimestamp($event['event_start'], $icalendarTimeHandler->_getUserTimeZone(icms::$user));
	$endEvent = xoops_getUserTimestamp($event['event_end'], $icalendarTimeHandler->_getUserTimeZone(icms::$user));
...
function _makeXMLSlideshowConf($options) {
	...
	foreach($options['images'] as $images) {
		...
		$value = $doc->createTextNode(ICMS_URL.'/uploads/'.$images->getVar('image_name'));
	    $src->appendChild($value);
	}

	// get completed xml document
	$xml_string = $doc->save(ICMS_ROOT_PATH.'/cache/icalendarSlideShowParam.xml');

}

function bIcalendarMinicalShow($options) {
	
	if(!defined('CALENDAR_ROOT')) {
		define('CALENDAR_ROOT', ICMS_ROOT_PATH.'/modules/icalendar/class/pear/Calendar/');
	}
	...
	if($options[0] == 1) {
		$imageHandler =& icms::handler('image');
	...
	$icalendarModuleConfig = $icalendarConfig->getModuleConfig();
	...
	$icalendarTimeHandler = IcalendarTime::getHandler();
	...
	*/
	...
	foreach($events as $event) {
		...
		if(!$event['event_isrecur']) {
			...
			bIcalendarMinicalAddEventToArray($event, $eventsArray, $icalendarTimeHandler, $startMonth, $endMonth);
			...
			foreach($recurEvents as $recurEvent) {
				bIcalendarMinicalAddEventToArray($recurEvent, $eventsArray, $icalendarTimeHandler, $startMonth, $endMonth);
			}
	...
	*/
	...
	$selectedDays = array(new Calendar_Day(
		date('Y', xoops_getUserTimestamp(time(), $icalendarTimeHandler->_getUserTimeZone(icms::$user))),
		date('n', xoops_getUserTimestamp(time(), $icalendarTimeHandler->_getUserTimeZone(icms::$user))),
		date('j', xoops_getUserTimestamp(time(), $icalendarTimeHandler->_getUserTimeZone(icms::$user)))
	));

	// Build calendar object
	$monthCalObj = new Calendar_Month_Weeks($year, $month, $icalendarModuleConfig['week_start_day']);
	$monthCalObj->build();
	...
	for($i=0;$i<$icalendarModuleConfig['week_start_day'];$i++) {
	...
	$navig = array(
					...
					'name'=>$icalendarTimeHandler->getFormatedDate($icalendarModuleConfig['nav_date_month'], $monthCalObj->getTimestamp())
			);
	$ret = array(
				'imageParam'=>$imageParam,
				'displayLink'=>$displayLink,
				'submitText'=>_MB_ICALENDAR_SUBMIT_LINK_TEXT,
				'tableRows'=>$tableRows,
...
function bIcalendarMinicalEdit($options) {

	$catHandler = icms_getModuleHandler('cat', 'icalendar');
	$cats = $catHandler->getAllCat(icms::$user, 'icalendar_cat_view');

	$imageCatHandler = icms::handler('imagecategory');

	$form = _MB_ICALENDAR_DISPLAY_IMG."&nbsp;\n";
	if($options[0] == 1) {
	...
	$form .= "</select><br /><br />\n";
	$form .= _MB_ICALENDAR_SS_WIDTH." <input name='options[2]' type='text' value='".$options[2]."' size='3' maxlength='3'> "._MB_ICALENDAR_PX."<br />";
	$form .= _MB_ICALENDAR_SS_HEIGHT." <input name='options[3]' type='text' value='".$options[3]."' size='3' maxlength='3'> "._MB_ICALENDAR_PX."<br />";
	$form .= _MB_ICALENDAR_SS_TRANS_TIME." <input name='options[4]' type='text' value='".$options[4]."' size='2' maxlength='2'> "._MB_ICALENDAR_SECONDES."<br />";
	$form .= _MB_ICALENDAR_SS_PAUSE_TIME." <input name='options[5]' type='text' value='".$options[5]."' size='2' maxlength='2'> "._MB_ICALENDAR_SECONDES."<br />";
	$form .= _MB_ICALENDAR_SS_NB_PHOTOS." <input name='options[6]' type='text' value='".$options[6]."' size='2' maxlength='2'><br /><br />";
	$form .= _MB_ICALENDAR_DISPLAY_MONTH."&nbsp;\n<select name='options[7]'>\n";
	if($options[7] == -1) {
		$form .= "<option value=\"-1\" selected=\"selected\">"._MB_ICALENDAR_PREVIEW."</option>";
		$form .= "<option value=\"0\">"._MB_ICALENDAR_CURRENT."</option>";
		$form .= "<option value=\"1\">"._MB_ICALENDAR_NEXT."</option>";
	} else if($options[7] == 1) {
		$form .= "<option value=\"-1\">"._MB_ICALENDAR_PREVIEW."</option>";
		$form .= "<option value=\"0\">"._MB_ICALENDAR_CURRENT."</option>";
		$form .= "<option value=\"1\" selected=\"selected\">"._MB_ICALENDAR_NEXT."</option>";
	} else {
		$form .= "<option value=\"-1\">"._MB_ICALENDAR_PREVIEW."</option>";
		$form .= "<option value=\"0\" selected=\"selected\">"._MB_ICALENDAR_CURRENT."</option>";
		$form .= "<option value=\"1\">"._MB_ICALENDAR_NEXT."</option>";
	}
	$form .= "</select><br /><br />\n";
	$form .= _MB_ICALENDAR_DISPLAY_SUBMIT_LINK."&nbsp;\n";
	if($options[8] == 1) {
		$form .= "<input name='options[8]' value='1' type='radio' checked='checked'>&nbsp;"._YES."\n<input name='options[8]' value='0' type='radio'>&nbsp;"._NO."<br /><br />";
	} else {
		$form .= "<input name='options[8]' value='1' type='radio'>&nbsp;"._YES."\n<input name='options[8]' value='0' type='radio' checked='checked'>&nbsp;"._NO."<br /><br />";
	}
	array_shift($options);
	array_shift($options);
	array_shift($options);
	...
	$form .= _MB_ICALENDAR_CAT_TO_USE."<br /><select name=\"options[]\" multiple=\"multiple\" size=\"5\">";
	if(array_search(0, $options) === false) {
		$form .= "<option value=\"0\">"._MB_ICALENDAR_ALL_CAT."</option>";
	} else {
		$form .= "<option value=\"0\" selected=\"selected\">"._MB_ICALENDAR_ALL_CAT."</option>";
	}
	foreach($cats as $cat) {
...
function bIcalendarSpotlightShow($options) {

}

function bIcalendarSpotlightEdit($options) {

}

function bIcalendarUpcomingShow($options) {
	...
	$eventHandler->formatEventsDate($events, $icalendarModuleConfig['event_date_month']);

	return $events;
}

function bIcalendarUpcomingEdit($options) {

	$catHandler = icms_getModuleHandler('cat', 'icalendar');

	$cats = $catHandler->getAllCat(icms::$user, 'icalendar_cat_view');

	$form = _MB_ICALENDAR_DISPLAY."&nbsp;\n";
	$form .= "<input name=\"options[0]\" size=\"5\" maxlength=\"255\" value=\"".$options[0]."\" type=\"text\" />&nbsp;"._MB_ICALENDAR_EVENTS."<br />";
	$form .= _MB_ICALENDAR_TITLE_LENGTH." : <input name=\"options[1]\" size=\"5\" maxlength=\"255\" value=\"".$options[1]."\" type=\"text\" /><br />";
	array_shift($options);
	array_shift($options);
	$form .= _MB_ICALENDAR_CAT_TO_USE."<br /><select name=\"options[]\" multiple=\"multiple\" size=\"5\">";
	if(array_search(0, $options) === false) {
...
function bIcalendarDayShow($options) {
...
function bIcalendarDayEdit($options) {
...
function bIcalendarNewShow($options) {
...
function bIcalendarNewEdit($options) {
...
function bIcalendarRandomShow($options) {
...
function bIcalendarRandomEdit($options) {
