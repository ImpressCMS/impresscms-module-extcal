...
$xoopsOption['template_main'] = 'icalendar_calendar-month.html';
...
$lang = array(
	...
	'calmonth'=>_MD_ICALENDAR_NAV_CALMONTH,
	'calweek'=>_MD_ICALENDAR_NAV_CALWEEK,
	'year'=>_MD_ICALENDAR_NAV_YEAR,
	'month'=>_MD_ICALENDAR_NAV_MONTH,
	'week'=>_MD_ICALENDAR_NAV_WEEK,
	'day'=>_MD_ICALENDAR_NAV_DAY,
	'feed'=>_MD_ICALENDAR_FEED
);
...
for($i=1;$i<13;$i++) {
	$monthSelect->addOption($i, $icalendarTimeHandler->getMonthName($i));
}
...
$navig = array(
			'prev'=>array(
						...
						'name'=>$icalendarTimeHandler->getFormatedDate(icms::$module->config['nav_date_month'], $pMonthCalObj->getTimestamp())
			),
			'this'=>array(
						'uri'=>'year='.$monthCalObj->thisYear().'&amp;month='.$monthCalObj->thisMonth(),
						'name'=>$icalendarTimeHandler->getFormatedDate(icms::$module->config['nav_date_month'], $monthCalObj->getTimestamp())
			),
			'next'=>array(
						'uri'=>'year='.$nMonthCalObj->thisYear().'&amp;month='.$nMonthCalObj->thisMonth(),
						'name'=>$icalendarTimeHandler->getFormatedDate(icms::$module->config['nav_date_month'], $nMonthCalObj->getTimestamp())
			)
);
