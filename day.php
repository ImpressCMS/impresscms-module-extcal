...
$xoopsOption['template_main'] = 'icalendar_day.html';
...
$navig = array(
			'prev'=>array(
						...
						'name'=>$icalendarTimeHandler->getFormatedDate(icms::$module->config['nav_date_day'], $pDayCalObj->getTimestamp())
			),
			'this'=>array(
						'uri'=>'year='.$dayCalObj->thisYear().'&amp;month='.$dayCalObj->thisMonth().'&amp;day='.$dayCalObj->thisDay(),
						'name'=>$icalendarTimeHandler->getFormatedDate(icms::$module->config['nav_date_day'], $dayCalObj->getTimestamp())
			),
			'next'=>array(
						'uri'=>'year='.$nDayCalObj->thisYear().'&amp;month='.$nDayCalObj->thisMonth().'&amp;day='.$nDayCalObj->thisDay(),
						'name'=>$icalendarTimeHandler->getFormatedDate(icms::$module->config['nav_date_day'], $nDayCalObj->getTimestamp())
			)
);
